...
<body>    
  ...
  <div id="conteneur">	
    ...
    if (isset($_POST['rapports7']))
    {
				...
				if ($db_link)
				{
			          ...
			          if ($jeu) {
					    ...
					    $requete=	"delete from rapport where jeu_id='$jeu_id' and type='7'";
					    $result = mysql_query($requete);

				    	echo "****************************************". $jeu_id . "<br>\n";
				      $chaine7 = $_POST['rapports7'];
				      $array = explode(PHP_EOL, $chaine7);
				      foreach ($array as &$value) {
				 	   	...
				 	   	if (isset($rang) && $rang!=0) {
							 $requete=	"insert into rapport (jeu_id, type, rang, nombre, rapport, commentaire) values ('$jeu_id', '7', '$rang', '$gagnant', '$rapport', now())";
   							$result = mysql_query($requete);
						}
    ...
    /*	
      $chaine7 = $_POST['rapports7'];
      $chaine7 = str_replace("\\","",$chaine7);
      $chaine7 = stripslashes($chaine7);

      $fd = @fopen("scripts/rapports_generes_7.php", "w"); // on ouvre le fichier en  criture
      if ($fd) 
      {
        fwrite($fd,$chaine7); 
        fclose($fd);
    ...
    {
      $fp = @fopen("scripts/rapports_generes_7.php","r"); //lecture du fichier
      if ($fp) 
      {
          while (!feof($fp))
          { 
            $chaine7 .= fgets($fp, 4096); 
          }  
          fclose($fp);
          $chaine7 = str_replace("\\","",$chaine7);
          $chaine7 = stripslashes($chaine7);
      ...
      <div id="contenu">
		...
		foreach($jeux as $jeu) 
		{
			...
			// LCA $texte = '<h3>Jeu à 7</h3><p>Rapports officiels - MAJ le ' . date('j M Y à H:i:s') . '</p>';
			//echo "<br>";
			$rangs = $jeu['rangs'];
			...
			echo '<form method="post" action="admin_rapports7.php" class="formconnexion" >';
			echo '<textarea name="rapports7" style="display:none;">'. $texte .'</textarea>';
        ...
        <div id="mini">
          <form method="post" action="admin_rapports7.php" class="formconnexion" >
            <textarea cols="80" rows="15" name="rapports7"><?php echo $chaine7; ?></textarea>
            <br />
